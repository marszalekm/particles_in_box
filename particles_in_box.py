...
class ParticlesInBox:
    ...
    def create_origin(self):
        ...
        for n in range(self.n_mol):
            xyz.write("%.2f %.2f %.2f\n" % (originlist[n][0], originlist[n][1], originlist[n][2]))
        return origin, xyz
    ...
    def step_to_file(self, pstep, path, xyz):
        ...
        xyz.write("\n# Step number %d\n" % pstep)
        for n in range(self.n_mol):
            xyz.write("%.2f %.2f %.2f\n" % (pathlist[n][0], pathlist[n][1], pathlist[n][2]))
